...
use Yzh\Config;
use Yzh\BizlicGxV2H5APIServiceClient;
...
try {
    $bizH5APIClient = new BizlicGxV2H5APIServiceClient($config);
} catch (\Exception $e) {
    die($e->getMessage());
}

// 工商实名信息录入
$request = new GxV2H5APIPreCollectBizlicMsgRequest(array(
    ...
    'id_card_validity_start' => '2022-02-22',        // 身份证有效期开始时间
    'id_card_validity_end' => '2042-02-22'           // 身份证有效期结束时间
));
...
$response = $bizH5APIClient->gxV2H5APIPreCollectBizlicMsg($request);
...
// 预启动
$request = new GxV2H5APIGetStartUrlRequest(array(
    'dealer_id' => $test_var['app_dealer_id'],      // 平台企业 ID
    'broker_id' => $test_var['app_broker_id'],      // 综合服务主体 ID
    'dealer_user_id' => 'userId1234567890',         // 平台企业端的用户 ID，在平台企业系统唯一且不变
    'client_type' => 2,                             // 客户端类型
 ...
 */
$request->setRequestID("requestIdExample123456789");
$response = $bizH5APIClient->gxV2H5APIGetStartUrl($request);
if ($response->isSuccess()) {
    // 操作成功
    $data = $response->getData();
    var_dump($data);
...
// 查询个体工商户状态
$request = new GxV2H5APIGetAicStatusRequest(array(
    'dealer_id' => $test_var['app_dealer_id'],      // 平台企业 ID
    'broker_id' => $test_var['app_broker_id'],      // 综合服务主体 ID
    'open_id' => 'userId1234567890',                // 用户唯一标识
    'dealer_user_id' => 'userId1234567890',         // 平台企业端的用户 ID，在平台企业系统唯一且不变
    'real_name' => '张三',                           // 姓名
    'id_card' => '11010519491231002X',              // 身份证号码
));
 ...
 */
$request->setRequestID("requestIdExample123456789");
$response = $bizH5APIClient->gxV2H5APIGetAicStatus($request);
if ($response->isSuccess()) {
    // 操作成功
    $data = $response->getData();
    var_dump($data);
