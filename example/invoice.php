...
$request = new ApplyInvoiceRequest(array(
   'dealer_id' => $test_var['app_dealer_id'],   # 平台企业ID(必填)
   'broker_id' => $test_var['app_broker_id'],   # 综合服务主体ID(必填)
   'invoice_apply_id' => 'apply123456789',      # 发票申请编号（必填）
   'amount' => '1.0',                           # 申请开票⾦额（必填）
   'invoice_type' => '2',                       # 发票类型（必填）1:专票 2:普票
   'bank_name_account' => '',                   # 开户⾏及账号（选填，不填使⽤默认值）
   'goods_services_name' => '*测试分类*test测试内容', # 货物或应税劳务、服务名称 (选填，不填使⽤默认值)
